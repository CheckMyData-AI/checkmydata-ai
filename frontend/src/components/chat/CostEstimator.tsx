  ...
  return (
    <div className="flex items-center gap-2 text-[11px] text-text-tertiary">
      ...
      >
        ...
        {showTooltip && (
          <div className="absolute bottom-full left-0 mb-1.5 z-50 w-52 bg-surface-2 border border-border-default rounded-lg p-2.5 text-[10px] text-text-primary shadow-xl">
      ...
      {estimated_cost_usd != null && estimated_cost_usd > 0 && (
        <span className="px-1.5 py-0.5 rounded bg-surface-2 text-text-secondary">
          ~${estimated_cost_usd < 0.01 ? estimated_cost_usd.toFixed(4) : estimated_cost_usd.toFixed(2)}
        </span>
      )}

      <div className="flex-1 max-w-[80px] h-1 bg-surface-2 rounded-full overflow-hidden">
        <div
