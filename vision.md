...
- **A data warehouse or ETL pipeline** — the system reads from existing databases; it does not transform them, move them, or build a queryable copy of them. Two deliberate carve-outs, and they are named here because the Terms and Privacy pages already disclose both while this section did not (BIZ-10) — two of the product's own promise documents contradicting each other about the same behaviour, in the one described as load-bearing:
  - **External report APIs** (Google Analytics 4, App Store Connect, Google Play) are quota-limited, lag by days, and expose no query language, so the reports they return are cached per connection with provenance and a TTL — enough to make them answerable, never a general-purpose warehouse (see `docs/adr/0001-external-report-cache.md`).
  - **A bounded trace of what was answered**, from the production database: up to `CHAT_RAW_RESULT_ROW_CAP` rows (default 500) of the result behind each answer, stored in that message's metadata so the answer can still show its own evidence; and per-column value samples in the database index, which is how the agent knows that `status` holds `'shipped'` rather than `3`. Both are bounded, both are scoped to the connection they came from, and neither is queryable as a data source — the agent always re-reads the customer's database to answer. "Never stores production data" was the older, simpler sentence and it stopped being true the day either shipped.
