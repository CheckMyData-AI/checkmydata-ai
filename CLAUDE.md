...
- **Tests**: **6,790 total** — 6,108 backend collected + 682 frontend Vitest across 92 files (measured 2026-08-19: `pytest tests/ --collect-only -q`, `npx vitest run`). The CI coverage gate `fail_under` is **72%**; re-measure with `pytest --cov=app` rather than quoting a figure from an older run.
