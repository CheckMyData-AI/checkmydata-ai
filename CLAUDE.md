...
- MCP server (`backend/app/mcp_server/`) is **off by default** (`MCP_ENABLED`); when on, its API key is bound to a real user via `MCP_API_KEY_USER_ID`. MCP resources use the same principal/ownership checks as MCP tools.
