...
- SSH: `SSH_HOST_KEY_POLICY` defaults to `tofu` and **fail-closes** to `strict` on unknown values. `SSH_PRE_COMMAND_ALLOWLIST_ENABLED` (default on) gates **only** the command-shape allowlist (`export`/`NAME=`/`source`/`cd`). Since 2026-08-21 (F-SSH-03) the count cap, length cap, non-empty-string check and shell-metacharacter screen apply whether it is on or off — it used to disable all five, so the documented "emergency hatch" also opened the injection surface. Opening it logs once per process. All exec-template values are shell-escaped, `db_port` included (F-SSH-04). Security-sensitive: `backend/app/connectors/ssh_tunnel.py`, `app/services/ssh_key_service.py`, `app/connectors/ssh_pre_commands.py`.
