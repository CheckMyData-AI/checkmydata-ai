...
| S-14 | **On a shared connection, wait for a test's background work — never cancel it.** Under `StaticPool` every session is one SQLite connection; a task cancelled mid-statement makes SQLAlchemy invalidate it, and the replacement is a NEW, EMPTY `:memory:` database. My first B-21 drain cancelled and turned one flaky test into 263 failures and 965 errors on CI — while the same code passed 702/702 locally, because whether the cancel lands mid-statement is timing. So a local pass of a timing-sensitive change is not evidence; CI's run is. And a harness change that can empty a shared resource must check that resource afterwards (`_assert_schema_survived`) so it fails at the cause. | 2026-09-26 (#439) | never — cancellation on a shared connection is the shape |
