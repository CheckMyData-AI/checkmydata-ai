...
| T00-mem | deliver | Web memory: the process needs 525 MB for one answer on a 512 MB dyno — BM25 corpus held in memory (32 531 docs, +93 MB), the ONNX embedder loaded per web process (+138 MB with a request), 294 MB of imports. Measure each, make the web path fit 1X again | V1 finding 2026-09-17 | todo | |
