...
| B-16 | deliver | F-K1 — batch table analysis maps tool calls to tables by position and skips an empty-args call without advancing, so later descriptions land on the wrong table | `docs/audits/2026-09-23-recent-work-audit.md` §3.1 | done — #408; nightly proof in V3b | `tests/unit/knowledge/test_batch_table_analysis_maps_by_name.py`, 5 tests, all failing on the old code for the defect's reason |
...
| T07d | deliver | PRJ-07 S-08 remainder — per-connection `db_index` as child jobs with their own ceilings, so four connections do not share one 7 200 s night | audit §PRJ-07 | done — #438 | chain of `run_daily_connection_sync` jobs (own ceiling 7200 s), one `daily_sync` row per connection; one-connection projects unchanged (production's only shape, so the new path has no production run yet); tests: chain queued, single stays inline, fallback inline, a failing link still passes the night on |
| B-28 | deliver | The sync-history panel read the retired KnowledgeSyncRun row | found scoping T07d | done — #437 | one fixture pins the row for both suites; SCN-064 FAIL → fixed |
| B-21 | deliver | The intermittent `test_learnings_api` StaleDataError | backlog B-21 | done — #439 | leftover background tasks on the shared SQLite connection; drained in `db_session` teardown; guard test red without it |
...
| V3b | verify | What only a nightly can prove: B-16 (no `unknown or ambiguous` warnings, fallback count), O-1 (cost per night vs $0.61), B-17 (rivalry caveats kept or refreshed), T07 (the 22:00 UTC wave dispatched once), B-25 on the next deploy | — | done 2026-09-26 | Three nightlies read from the database (the log holds hours, and there is no drain). **T07:** one wave per night 09-23/24/25, 4 runs each, 12/12 `completed`. **O-1:** background tokens per night 1.72M → 1.13–1.18M (−33%), cost $0.61 → $0.12–0.25 (the price moves on its own; tokens are the measure). **B-16:** 0 of 95 fully-covered tables carry notes about a foreign column. **B-17:** three `MEASURED (rivalry)` caveats rewritten 2026-09-25 22:28. **B-25:** one chat trace since the deploy, routed `query/moderate`, not the default — too few to conclude; the routing eval is the proof |
...
| T09 | deliver | B-02 — replace the integration harness's open-transaction isolation so the suite runs on PostgreSQL | backlog B-02 | done — #425; CI job waits on Q2 | 698/698 on PostgreSQL 17 + pgvector (231 s) and SQLite (158 s); found a production defect (`resolve_account_key` on the request session → SAVEPOINT) and `llm_credit` missing from `app/models/__init__.py` |
| B-19 | deliver | F-T1 — a process-wide rebroadcast flag muted local trace events | backlog B-19 | done — #426 | `tests/unit/core/test_a_rebroadcast_does_not_mute_local_events.py`, red on the old code |
| B-18 | deliver | UX verification rot — re-audit the 14 oldest scenarios | backlog B-18 | done — #427 | 7 PASS / 7 PARTIAL / 0 FAIL; six defects fixed, each test red on the old code; stale 94 → 92 |
| V3 | verify | T07–T09, B-18, B-19, B-26, B-27 and release 1.18.0 landed | — | done 2026-09-26 on v459 / web v388 | API reports `checkmydata-ai` **1.18.0**; `alembic_version` = repo head `e1f2a3b4c5d7`; markers present (CORS `X-Result-Capped`, readiness `repo_indexing`, `pipeline_continuation_context`, the honest continuation prompt); six new UI strings found in the served chunks, lazy ones included; fonts served from `/_next/static/media` as `font/woff2`, no Google Fonts in HTML or CSS; `/`, `/app`, `/login`, `/pricing`, `/verify-email`, `/about` all 200 under 0.61 s; `/api/health` ok; 0 errors / 5xx / H1x / R1x in 150 log lines after the release; web 311 MB, worker 255 MB; 0 open `error_log` rows; 0 active indexing runs at every merge |
...
| T12 | deliver | PRJ-06 — indexing pipeline as a declared DAG with a resumable full rebuild | audit §PRJ-06 | in progress — T12a done (#441): a full rebuild resumes, pinned to its checkpoint's tree (S-11). Remainder: `Step` objects, per-step sessions, run-keyed `save()`, per-run doc/token cap, `to_thread` for the four inline loops | T12a: `test_a_full_rebuild_resumes.py` (10); production acceptance (interrupted rebuild < 0.5× cold) read on the next restart that lands on a rebuild |
...
- **Cycle 4 (2026-09-26)** — #428 (cycle-3 consolidation), #429 (PostgreSQL CI job), #430 (reconciler test isolation), #431 (B-26), #432 (B-27: the second re-audit batch, 12 scenarios), #434 (fonts self-hosted after a Google Fonts hiccup failed the #431 deploy; a `node_modules` symlink I had committed in #432, untracked), and **release 1.18.0** (B-20). V3 for #425–#427 read on production v457: backend markers present, the new UI strings in the served bundle, no errors after restart. One failure was mine and is now a rule: a unit test that uses `with TestClient(app)` runs the app lifespan and leaked state into 27 later tests. **Operator questions open: 0** (Q1 resolved on its own; Q2 answered by pushing over SSH).
- **Cycle 5 (2026-09-26)** — #436 (V3 of release 1.18.0 recorded), #437 (B-28: the sync-history panel read a row the API no longer returned — found while scoping T07d, and both suites were green), #438 (T07d: per-connection nightly jobs), #439 (B-21: background work outliving its test on the shared SQLite connection — the first fix cancelled and broke CI, the second waits), #441 (T12a: a full rebuild resumes). **Operator questions open: 0.**
