...
*Counted 2026-08-21, not estimated: 54 open rows and 55 struck
...
| F-KNOW-12 | 🟡 | BM25 snapshots still live on the dyno's ephemeral disk — hybrid retrieval really is dense-only after every restart until a reindex. F-KNOW-07 made that **measurable** (`retrieval_degraded_total{leg="bm25",reason="no_snapshot"}`) and explicitly did not fix it. Fix is shared storage or rebuild-on-boot; decide with the metric in hand, not before. |
