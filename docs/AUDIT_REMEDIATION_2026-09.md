...
| 2.2 | Feed symbol chunks into the BM25 corpus | M | todo | Re-verified open 2026-09-05 with both sides: `_run_bm25_build` (`pipeline_runner.py:1392`) rebuilds the snapshot **from `KnowledgeDoc` rows**, while `code_symbol_chunker.py:218` writes symbol chunks under `sym:` ids into the **vector store only** — and says so in its own comment, that BM25 docs are "built separately by the bm25_build stage from the same underlying documents". They are not the same documents. **The two legs of the hybrid retriever index different corpora**, so a query naming a function or class — the natural way to ask about code — reaches the lexical leg only through prose. Directly on the stated target: vector search over a repository to find parts of an implementation **Architecture pass 2026-09-05: the row understates it by an order of magnitude.** `KnowledgeDoc.content` is LLM-generated documentation, not source (`pipeline_runner.py:1172` stores `generated_content`), so **the lexical leg has never indexed a line of code** — an identifier reaches it only when the model wrote it into a summary. Two constraints read from the code settle the design: the snapshot must be rebuildable from Postgres with no clone (`bm25_local_reconcile`'s own contract; breaking it is the F-KNOW-12 defect), so a symbol document is name+signature+docstring from `code_graph_symbols` and **not** the body, which Postgres does not hold; and `HybridRetriever._fuse` merges on `doc_id`, so the entry id must equal the chunker's, or one symbol becomes two documents and its rank **splits instead of reinforcing**. Ships after 2.16 and 2.3, with 2.10 as a hard prerequisite. |
...
| 2.10 | Prefix-match on delete; symbol chunks are never pruned | M | todo | Re-verified open 2026-09-05, and the mechanism is sharper than the title: it is **two metadata keys for one concept**, not a missing prefix match. `chunker.py:98` gives a prose chunk `source_path`; `code_symbol_chunker.py:87-94` gives a symbol chunk `path` (`REQUIRED_CHUNK_METADATA_KEYS` does not contain `source_path` at all). Both `delete_by_source_path` implementations — `vector_store.py:295` and `pgvector_store.py:283` — filter on `metadata.source_path`, so a re-index replaces a file's prose chunks and its symbol chunks **accumulate forever**. The store ends up holding symbols for code that no longer exists, and dense retrieval returns them: the agent can cite a function deleted months ago. Pairs with 2.2 and 2.3 — all three are the symbol-chunk seam and should ship together **Sequencing found by the architecture pass, and it is a gate rather than a preference:** the symbol chunk id must become a pure function of the symbol (`path` + `uid` + `start_line`) before it can be reproduced from Postgres for the BM25 corpus — `uid_suffix` is today a **global** counter over `parsed_files` that also skips files whose source is unreadable, so it is not derivable from the database at all, and an unrelated file appearing earlier churns the ids of duplicates in later ones. But changing the id while symbol chunks are unprunable orphans every existing vector permanently: `force_full` only resets `last_sha`, nothing clears the collection, and `code_symbol_chunker` never deletes. So this row ships **first, in the same change**. Clearing the collection up front was considered and rejected — a 12 039 s rebuild on a worker with a SIGKILL history would leave a partial index where a crash today leaves a stale but complete one. The fix itself needs no re-index: the `path` key is write-only (nothing in `app/` reads it), so it is renamed to `source_path` and deletion accepts the legacy key, which sweeps the existing orphans on the next run. |
...
- **The suppression ratchet caught its author, and the answer was to delete the
  ...
  and its regression tests hold. The two tools disagree about what a mention is. **It then caught the same author again, on a different kind** — a `# type: ignore[attr-defined]` in `bm25_corpus.py`, written only because the parameter was typed `object` rather than `Iterable[StoredDoc]`. Twice in one day, twice avoidable, both fixes under a minute. That is the argument for never raising the ceiling on reflex: a ratchet whose ceiling moves whenever it fires is a counter, not a gate.
