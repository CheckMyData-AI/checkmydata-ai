...
| 0.4 | Turn selling on for real — three `STRIPE_PRICE_*` + `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` | S | **blocked** | **Measured 2026-09-05, and the row's framing understates it.** Production has `BILLING_ENABLED=true` with **no Stripe variables set at all** — `heroku config` shows `BILLING_ENABLED` and nothing else billing-related. So the routes are not 404: they are live and broken. `_stripe()` raises `BillingError("Stripe is not configured")` on the first call, which means a visitor who clicks upgrade gets an error rather than a missing page — **the product looks purchasable and is not**. Five variables are needed: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and prices for the two live plans plus the top-up (`STRIPE_PRICE_BASE`, `STRIPE_PRICE_SCALE`, `STRIPE_PRICE_CREDIT_TOPUP`; `_price_id_for` reads the DB column first and falls back to the env). The webhook endpoint is `POST /api/billing/webhook`. Operator-only: the keys are never entered or stored by the agent, and the endpoint is created in the Stripe dashboard because the API response carries the signing secret. **Interim option, one command:** `BILLING_ENABLED=false` makes the routes 404 again, which is honest, until the keys are in place |
...
| 1.9 | Pass `tracker` at the three `HybridRetriever` sites | S | done | **The row's one-line title would have shipped a cross-request leak.** `emit_retrieval_degraded` records its metric unconditionally but emits the workflow event — the one rendering the reader-facing "keyword search is unavailable" line — only when a tracker is present, and none of the three sites passed one. The counter grew while an answer built on one leg of two rendered exactly like one built on both. Third occurrence of one shape today: a rule implemented in one layer, unreachable because another never sends the value (cf. 1.7's seal, 2.9's gate). **The fix is per-CALL.** `chat.py:62` builds `ConversationalAgent()` at module scope and each site caches its retriever on `self`. `WorkflowTracker` is a bus and is safe to hold; `workflow_id` is not — it names one request's stream, and `HybridRetriever.__init__` accepts it. `query()` takes both per call, `wf_id` threads through four signatures following the loader's own idiom, and a guard forbids `workflow_id=` at the three constructor calls. **That guard's first draft forbade `tracker=` and was wrong** — reading `WorkflowTracker` corrected it. `tests/unit/knowledge/test_degradation_reaches_the_reader.py` (11 cases, 7 red before) |
...
| # | Task | Size | Status | Evidence |
|---|---|---|---|---|
| 2.1 | Render `Artifact.title`; dedup on `(title, summary)` | S | done | `render_context_block` emitted `a.summary` and never `a.title`, and the titles it dropped are the identifying half of every artifact: the table name, the rule name, the source path, the lineage endpoints, the learning's subject. The model was handed descriptions of tables without their names and asked which table holds what — on the default path, since `context_planner_enabled` is on. **The budget already paid for them:** `_make_default_sizer` counts `title + summary` and calls them "the fields exposed to the LLM", so the pack was charged for text it never sent and fitted fewer artifacts than it could. Fourth occurrence this session of one part of the system implementing a contract another does not honour. **The dedup was the sharper half:** a table's summary is `business_description or ""`, so every undescribed table hashed to `""` and all but the first were dropped — a project with no generated descriptions got one blank line where its table list should be. **RET-R15's intent survives, but not for the reason the fix first claimed:** its pair is two retrieval VIEWS of one entity whose titles differ by construction (`"Foo"` / `"Foo prose"`), which `test_w2_low_batch` proved by going red. The key now follows the kind — summary alone for `code_entity`/`rag_chunk`, the pair for distinct entities that merely share prose. `tests/unit/knowledge/test_context_names_what_it_shows.py` (9 cases, 7 red before) |
| 2.2 | Feed symbol chunks into the BM25 corpus | M | done | `corpus_entries_for_symbols` puts one lexical document per symbol — name, signature, decorators, docstring, path from `code_graph_symbols` — into the corpus both processes build. **Not the body, and that is a constraint:** the snapshot must be rebuildable on the web dyno from Postgres with no clone (`bm25_local_reconcile`'s contract; F-KNOW-12 is the cost of breaking it), and Postgres does not hold bodies. What is left is what a lexical index wants — identifiers, parameter names, docstring prose. The id comes from the shared `symbol_chunk_id`, so `HybridRetriever._fuse` reinforces one symbol instead of ranking two documents for it; `chunk_index=0` always, so a split symbol's later chunks stay dense-only. A symbol with no name is skipped: the identifier is what the document is for. `tests/unit/knowledge/test_symbols_reach_the_lexical_leg.py` (11 cases, 3 red before). SCN-047 updated |
...
| 2.4 | Run DataGate before the truncation return; widen hard checks past 21 keywords | M | done | **Two halves, and the second ships the opposite of what was asked.** *Order:* `evaluate` returned a truncation `warn` before running the hard checks, and the decision table called that intentional — backwards, since a capped result is the case most likely to carry a broken aggregate, so the truncation flag made the gate skip exactly those. Reordered; when both fire the block wins and its reason names the truncation too. The trap, which has its own test: `skip_data_gate` must no longer RETURN, or the pipeline path loses its truncation warning along with the gate. *Widening:* the audit proposed a money kind and **it is deliberately not added** — a negative revenue is a refund or a chargeback, `fail` is a hard refusal to use the result, and a gate that blocks it is a product that cannot answer "how much did we refund". `_DELTA_KEYWORDS` already demotes `percent_change` to `rate` for that same reason. What ships instead needs **no classifier**: `NaN`/`Infinity`, which arrive from a 0/0, an overflow or a bad cast, are unusable at any column name, and today are worse than unchecked because `_check_nulls` counts a NaN as a null. `tests/unit/test_truncation_does_not_hide_impossible_values.py` (14 cases, 7 red before) |
...
| 2.6 | `to_thread` the schema BM25 load off the request path | S | done | **Verified open before being closed, and the first reading was wrong.** `sql_agent` threads the BM25 *query* — `await retriever.aquery(...)`, with a comment saying it offloads CPU-bound work off the loop — and reached it through `retriever.has_index(connection_id)`, which is not threaded and is the **expensive half**: it resolves through `BM25Index.indexed_sha` to `load()`, which gunzips the snapshot and rebuilds `BM25Okapi` from the tokenised corpus. `aquery` then found it cached. So the call deliberately moved off the loop was already warm and the one doing the work was not. `bm25_local_reconcile.py:120` wraps the identical call — one call, two sites, threaded in one. Worst right after a deploy, when nothing is cached and every project's first question pays the deserialisation with the loop held. `tests/unit/test_schema_probe_is_off_the_request_path.py` (4 cases, 1 red before) |
...
| 2.8 | Alias-bind `check_required_filters` | M | done | The guard ran `pattern.search(query)` over the **raw** SQL, so a requirement was satisfied by the same characters anywhere — a comment, a string literal, or the other side of a join. Production carries 159 such predicates across 57 tables and soft-delete column names repeat between them, so an unbound match is the normal case rather than an exotic one. Two halves. *Scrubbing* is the session's recurring shape for the fourth time — one job, three implementations, one correct: `safety.py`'s dialect-aware scanner moved to the leaf `core/sql_text.py`, `sql_parser` lost its two quoting-blind regexes, and the guard stopped reading raw text. *Binding* requires the qualified form only when the query names more than one table; with one, a bare column is unambiguous and demanding `orders.deleted_at` would false-block most correct SQL — which costs an unsatisfiable repair loop measured at ~23 s per query. **The first implementation reproduced the defect it fixes**: blanking string literals made every predicate whose operand IS a literal (`status = 'active'`) unsatisfiable, and four cases in `test_required_filter_guard_reads_real_predicates.py` — the file kept precisely to stop unsatisfiable predicates — went red at once. The distinction is positional, not textual: a match that STARTS inside a literal is text; one that starts outside may have a literal as its operand. `tests/unit/test_required_filters_bind_to_their_table.py` (20 cases, 7 red before) |
...
| 2.10 | Prefix-match on delete; symbol chunks are never pruned | M | done | Two halves, both needed. **Renamed** the symbol chunk's `path` key to `source_path` so the divergence stops growing — safe because the key was write-only, nothing in `app/` read it. **Deletion accepts the legacy key** in both stores, because a rename fixes nothing already written and production holds ~25 000 symbol chunks under `path` that removal could not match; that half needs no re-index and sweeps them on the next run. pgvector was patched separately — `VECTOR_STORE_BACKEND=auto` resolves to it on Postgres, so a Chroma-only fix would have fixed only development. `tests/unit/knowledge/test_symbol_chunks_are_pruned.py` (7 cases, 5 red before) |
| 2.11 | Mongo nullability and FKs; ClickHouse primary keys | M | done | **The nullability half was a type that could not say "unknown".** `ColumnInfo.is_nullable` was `bool = True`; of twelve construction sites, four omit it — `mongodb.py:341`, `ssh_exec.py:525` and two in `entity_extractor.py` — and every one of those four genuinely cannot know: Mongo has no schema, the extractor is reading source. So the default was an assertion nobody made in every case that used it, and the prompt printed ` NULL` beside each column as fact. Now `bool | None = None`, the gap this codebase already closed for `reltuples < 0` (D14) and `depth_estimated` (L12). **Flipping the default was only half:** three of the four readers turned a falsy value into the OPPOSITE assertion (`"YES" if col.is_nullable else "NO"`), so `None` would have become NOT NULL — one lie for another. Only `schema_context_builder` survived, and by accident. **The ClickHouse half is a different shape:** `no_pk` fired on every table of an engine that has no primary keys, forever, and a warning on the normal path is what teaches a reader to ignore warnings. It now keys on `is_sort_key` — already captured by W4 and already rendered — which says the true thing rather than special-casing a dialect. **Mongo foreign keys are deliberately NOT added:** MongoDB has none, and inferring them from ObjectId references would assert a relationship the database does not enforce — which is this row's whole subject. `tests/unit/test_schema_does_not_assert_the_unknown.py` (7 cases, 6 red before) |
| 2.12 | SQLite dialect hints — the demo path has none | S | done | **Fixed as a class, not as a symptom.** `DIALECT_HINTS` is keyed on four engines while `ADAPTER_REGISTRY` accepts eight names, and the lookup is a `.get(db_type, generic)` — so `sqlite`, `postgresql` and `mongo` all fell through silently. Adding one `sqlite` key would have left the other two, and a ninth accepted name would repeat it. The lookup now canonicalises through `canonical_dialect`, which lives beside the registry that defines the vocabulary, and a test walks every accepted name. `mcp` is excluded deliberately — a transport is not a dialect, and the generic fallback is the honest answer for it. `sqlite` matters most because `demo.py:101` creates it: **every first-touch demo ran with no dialect guidance**, so the model reached for `NOW()`, `ILIKE` and `::` casts that produce a syntax error rather than a slow query. The hints name the real differences and a test asserts that rather than the key's presence, because generic text would pass a presence check and still let the model write `NOW()`. `tests/unit/test_every_accepted_db_type_has_dialect_hints.py` (14 cases, 6 red before) |
...
| 2.17 | A symbol chunk id that depended on the order its file was parsed in | S | done | Found while designing 2.2. The id was `sym:{path}:{uid}{suffix}:{idx}` where `suffix` came from a counter kept **across every file** in the run, so a file appearing earlier shifted the suffix of every later duplicate — editing one file churned chunk ids belonging to another — and the id could not be rebuilt from the database at all, because replaying it means replaying which files were skipped for an unreadable source, which is recorded nowhere. Now a pure function of the symbol: `sym:{path}:{uid}@{start_line}:{idx}`, all three fields columns on `code_graph_symbols`. The uniqueness guard stays — Chroma rejects a whole batch on a duplicate and production lost nine batches of 200 to one such error — but it now **logs**, because a suffixed id is exactly the one the corpus builder cannot reproduce, and a silent fallback would make the legs disagree invisibly. Rides a fifth fingerprint axis, `SYMBOL_CHUNK_ID_SCHEMA`. `tests/unit/knowledge/test_symbol_chunk_id_is_reproducible.py` (11 cases, 2 red before) |
| 2.18 | A count column not called `count` is unchecked, and no candidate fix is safe | M | todo | Left open deliberately while closing 2.4, recorded rather than patched. `_COUNT_KEYWORDS` is `{count, cnt, qty, quantity, num, number}`, so `users`, `orders`, `sessions`, `clicks` and `signups` are counts where a negative is genuinely impossible and none match. Every candidate fix is either a list of nouns the next noun defeats, or a shape heuristic that cannot tell a count from an id from a year — and a false `fail` costs a refused answer, because the block appends a "DO NOT USE THIS RESULT" marker the model obeys. Needs a real idea, not a longer list |
| 2.19 | `test_expired_token_returns_400` failed once and could not be reproduced | S | todo | Seen 2026-09-05 in a full run: `assert 200 == 400`, i.e. the reset succeeded against a token whose expiry had been pushed into the past and committed. **Did not reproduce** — passes in isolation and across the full 680-test integration suite. Not attributable to that run's change, which touches `result_validation` and `data_gate` and nothing near auth. Recorded because an auth test that fails once a run is a liability either way: either the isolation is wrong or the expiry check is |
| 2.20 | Sync vector-store calls on four async paths, one of them the worker's heartbeat | M | done | All four wrapped in `to_thread`, matching `HybridRetriever` — the site that had it right, and the reason the others read as oversights rather than house style. **The detector was wrong twice and both are recorded in it:** its first draft knew only `to_thread(store.query, …)` and reported the lambda and closure fixes as defects; loosening it to "anything in a nested def is fine" would have stopped it catching what it exists for, so it exempts the bodies of exactly the functions handed to a thread, with a test per shape and one for the floor. Re-watched failing against the restored original after the loosening. `tests/unit/knowledge/test_sync_vector_calls_do_not_block.py` (10 cases, 4 red before) |
| 2.21 | `KnowledgeCatalogService._rag_artifacts` has no callers | S | todo | The sync twin of `_rag_artifacts_async` is dead code — no call site — so a reader comparing the two learns a distinction that does not exist |
| 2.22 | Dialect knowledge lives in three maps, two of them identical | S | todo | Found while closing 2.12 and updated in both places rather than unified, which is a postponement. `core/sql_text.py::sql_dialect_for` answers the LEXING dialect; `core/query_builder.py::DIALECT_HINTS` and `agents/prompts/sql_prompt.py::DIALECT_HINTS` are two copies of the PROMPT hints with identical keys. Fifth occurrence this session of one job with several implementations. **And `query_builder` is imported nowhere in `app/`** — only from tests, so one of the two copies is dead production code with a live suite, the third such case today (cf. `_rag_artifacts`, the empty CODEIDX placeholders). Deleting it needs a look at whether `QueryBuilder` itself is still wanted |
| 2.23 | A heartbeat that cannot write was logged at DEBUG | S | done | Found by measuring production, not by reading. **163 failed indexing runs and every single one carries `stale run reaped`** — 83 repo indexes, 80 daily syncs, not one other cause. That uniformity is the finding: the reaper's message names what it did and says nothing about why the beat stopped, and the thing that would name it — `_beat`'s exception handler — wrote to `logger.debug`, invisible at production's INFO. The writer opens its own session per beat (deliberately: a session driven from two tasks is a race), so it needs a pool slot every 30 s. Now WARNING, with the exception text and a **consecutive-failure count** — one missed beat is noise, ten in a row is a run's cause of death, and only the count separates them afterwards. Swallowing stays: the module's contract is that a heartbeat must not crash the run it protects. `tests/unit/test_heartbeat_failure_is_visible.py` (5 cases, 4 red before) |
| 2.24 | The pools are sized at twice what the pooler allows | S | todo | Measured 2026-09-05: `db_pool_size=5` + `db_pool_overflow=10` is 15 per process, web and worker are two processes, and Supavisor session mode caps the account at 15 total — the app is configured for 30. A new client is refused with `EMAXCONNSESSION` while a repo index runs, reproduced twice forty minutes apart with no diagnostic dynos of our own in flight. The product itself keeps serving (health does a real `SELECT 1` and answers in ~1 s) because existing pooled connections work; what is gone is **headroom** — scaling web, restarting the worker or any operational access fails for the hours an index takes. One command, reversible: `DB_POOL_SIZE=4 DB_POOL_OVERFLOW=2`. Needs a dyno restart, so it waits for the in-flight index |
| 2.25 | The nightly cron's ceiling is half the work it starts | M | todo | Arithmetic, not luck. The current production run is regenerating **all 759 documents** — a full rebuild, measured in CLAUDE.md at 12 039 s (3 h 20 m). `daily_knowledge_sync` runs the same pipeline under a 7 200 s ceiling. **A nightly cron cannot complete a full rebuild in principle**, so it reaches the ceiling and dies, which is consistent with 80 of 96 daily syncs failing. Two questions before a fix: why every run is full rather than incremental, and whether the ceiling should rise or the cron should refuse a full rebuild and say so |
