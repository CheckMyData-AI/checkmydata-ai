...
| A pipeline stage cannot carry the `report` / `date_from` / `date_to` hints the flat loop's tool accepts: `PlanStage` has no parameter field (`stage_context.py:56-65`), so a plan can only narrow the window through the stage question in prose. A real capability difference between the two paths that nothing recorded before this row. | backlog — needs a `params` field on `PlanStage`, which touches plan persistence |
