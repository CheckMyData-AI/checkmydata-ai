...
| 1.9 | Pass `tracker` at the three `HybridRetriever` sites | S | done | **The row's one-line title would have shipped a cross-request leak.** `emit_retrieval_degraded` records its metric unconditionally but emits the workflow event — the one that renders the reader-facing "keyword search is unavailable" line — only when a tracker is present, and none of the three sites passed one. So the counter grew while an answer built on one leg of two rendered exactly like one built on both. Third occurrence today of one shape: a rule implemented in one layer and unreachable because another never sends the value (cf. 1.7's seal, 2.9's gate). **But the fix is per-CALL, not per-construction.** `chat.py:62` builds `ConversationalAgent()` at module scope and each site caches its retriever on `self`, so the object outlives the request. `WorkflowTracker` is a bus and is safe to hold; `workflow_id` is not — it names one request's stream, and `HybridRetriever.__init__` accepts it. `query()` now takes both per call, `wf_id` threads through `assemble_knowledge_block` -> `build_context_pack` -> `get_context_pack` -> `_rag_artifacts_async` following the loader's own idiom (`has_mcp_sources(project_id, wf_id='')`), and a guard forbids `workflow_id=` at the three constructor calls. **The first draft of that guard forbade `tracker=` and was wrong** — reading `WorkflowTracker` corrected it, and a test that forbids the wrong thing teaches the wrong lesson. `tests/unit/knowledge/test_degradation_reaches_the_reader.py` (11 cases, 7 red before) |
...
| 2.20 | Sync vector-store calls on four async paths, one of them the worker's heartbeat | M | todo | Verified 2026-09-05, and the row title it replaces ("to_thread the schema BM25 load") named the wrong subsystem. `VectorStore` and `PgVectorStore` are entirely synchronous and `hybrid_retriever` wraps them in `to_thread`; `context_loader.py:432`, `knowledge_catalog_service.py:570`, `mcp_server/resources.py:140` and `pipeline_runner.py:447` do not, and all four sit in `async def`. The first is the **documented graceful-degradation path**, so the worse retrieval health gets, the harder the loop freezes — and it embeds the question through the bundled ONNX model while doing it. The fourth is the sharp one: blocking the worker's loop starves the `heartbeat_at` write, and this programme already recorded a live run reaped at 22:07 because its beat stopped. `make_vector_store()` at the MCP site also opens a psycopg pool on pgvector |
| 2.21 | `KnowledgeCatalogService._rag_artifacts` has no callers | S | todo | Found while verifying the row above. The sync twin of `_rag_artifacts_async` is dead code — `grep` finds no call site — so a reader comparing the two learns a distinction that does not exist. Small, but it is the sync/async pair that makes the async one look deliberate |
