...
| 2.20 | Sync vector-store calls on four async paths, one of them the worker's heartbeat | M | todo | `VectorStore` and `PgVectorStore` are entirely synchronous and `hybrid_retriever` wraps them in `to_thread`; `context_loader`, `knowledge_catalog_service`, `mcp_server/resources` and `pipeline_runner` do not, and all four sit in `async def`. The first is the **documented graceful-degradation path**, so the worse retrieval health gets the harder the loop freezes — while embedding the question through the bundled ONNX model. The fourth is the sharp one: blocking the worker's loop starves the `heartbeat_at` write, and this programme already recorded a live run reaped at 22:07 for a stopped beat |
