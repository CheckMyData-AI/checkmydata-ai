...
- **States covered:** loading, error, success; in the investigation modal: investigating, **slow** (after 90 s: "Still working — an investigation can take a few minutes. Keep this window open."), **no result after 10 minutes** ("Check again"), **cannot reach the server** after three failed reads in a row ("Try again") — before T04b (2026-09-23) the poll stopped silently after 60 s or at the first failed read and the modal stayed on "investigating"
...
- **UI elements:** source-type select ("Google Analytics 4"), credential select + "＋ new credential" affordance + "Check key" (SCN-114a), property-IDs input, property-timezone input (up to eight suggestions matching what has been typed — the browser's zone list is not a gate, because a runtime with trimmed ICU data knows only a handful and refusing a zone read off GA4 would be a refusal the user cannot act on), event-names input, currency-code input, backfill-days input, collection-hour select, "Collect automatically" toggle, Save button
