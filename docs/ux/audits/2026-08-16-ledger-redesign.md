...
- **Not verified, and it matters:**
  - **The gap-not-zero claim in SCN-124 is verified at the MODEL, not at the pixels.** `chart-series.ts:105` produces `null` for a missing datum and a test asserts it; whether Recharts draws a gap rather than dropping to the axis was not observed in a browser.
  ...
  - **Contrast is stated, not measured in situ.** The pack's own ratios are computed against its tokens; whether a given composed surface (a status word inside a tinted chip, say) clears AA on screen was not sampled.
