...
|---|---|---|
| ~~D1~~ | **Resolved 2026-09-07: several repositories per project.** | FLW-03 keeps its multi-repository branch and its index queue. The backend gains a repository dimension across the indexing pipeline, the code graph, the BM25 snapshot, the generated docs and GitAgent; `SCN-138`/`SCN-140`/`SCN-141` are in scope rather than candidates for retirement |
