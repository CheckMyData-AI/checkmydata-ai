...
### Fixed — the production build no longer depends on Google Fonts (#434)

- `next/font/google` downloads its CSS during `next build`, and on 2026-09-26 a malformed
  response failed the production image build — the deploy of #431 was skipped and
  production stayed on the previous release until the next one. Inter and Space Grotesk
  are self-hosted from `@fontsource-variable/*` now; the build needs nothing outside the
  lockfile.
- #432 had committed `frontend/node_modules` as a symlink to a path on the author's
  machine, through a `.gitignore` pattern that ignored only directories. Untracked, the
  pattern fixed, and a test refuses a tracked symlink that leaves the repository.
...
The tests build their schema with `Base.metadata.create_all`; production builds it with
Alembic. Where those two disagree, every test in the suite exercises a database that
does not exist — and the divergence is invisible from either side.

**Eighty-one columns were `NOT NULL` in the model and nullable in the migrated schema.**
`Mapped[str]` without `| None` makes SQLAlchemy infer `nullable=False` and the models
rely on that inference throughout; the migrations that created those tables passed
`server_default=…` and no `nullable=False`. A 2026-03 hardening pass closed eighteen
tables and stopped.

The models are the authority here, because they are what the application is written
against, what `create_all` builds, and what `autogenerate` compares to — three readers
of four. And the column list is **derived from `Base.metadata`, never written down**:
eighty-one entries across nineteen tables is a list that is stale the day it is typed,
and the next table added would quietly rejoin the divergence. Verified end to end
against a real PostgreSQL 17: the sweep tightened **81 columns** — the audit's own
figure, arrived at independently — and a model-versus-`information_schema` comparison
then reported **0 divergences**.

A column with no safe answer to "what does an existing NULL become" is skipped and
logged by name rather than filled with an invented value, and a fill that actually
touches rows logs at WARNING with the count: on a column that has carried a server
default since it was created the honest expectation is zero.

**Seven more, each small and each a different kind of wrong.** `audit_logs` was missing
from `app/models/__init__.py`, so the table was absent from the metadata Alembic reads —
the next autogenerated revision would have emitted `drop_table("audit_logs")`, for the
table the security work added to make sensitive auth actions durable.
`ix_code_graph_symbols_cluster` existed in a migration and in no model, so it was absent
from every `create_all` schema. `db_index.row_count` was a 32-bit `Integer` fed from
`reltuples::bigint` and ClickHouse `UInt64`, which the connectors deliberately widen on
the way in with nothing clamping between. `mcp_api_keys.token_hash` carried two btree
indexes, and the object named `ix_mcp_api_keys_token_hash` meant different things in the
two schemas. `uq_error_log_project_sig` was a unique index over a nullable column, and
both engines treat NULLs as distinct — so the dedup rule imposed nothing at all on
system-scoped errors, which is most of them; it coalesces now, as
`uq_indexing_runs_active_one` already did one table over, and the read-then-write that
feeds it catches `IntegrityError` instead of turning a race into a 500 while recording
an error. The `doc_embeddings` expression index covered `source_path` while the delete
path filters `source_path OR path`, so PostgreSQL could not build a `BitmapOr`.

**And the schema had zero CHECK constraints**, while `indexing_runs.status` is the
predicate of the single-active-run guard: the partial unique index is scoped by a
literal list of three status strings, so a value outside the vocabulary escaped the
guard entirely while still reading as an active run to every Python caller.
...
- `docs/ux/scenarios.md` said `implemented` for all **127** scenarios and `PASS` for 125.
  Both were read as current, and neither was: **110 of the 127 carried an audit date of
  2026-07-19** while 152 commits had landed on `main` since, including changes to
  user-facing behaviour. Only **22 of 127** were referenced anywhere in code or tests, so
  for the other 105 there was no anchor a machine could check an implementation by.
  "100% implemented, 98% PASS" in that state is an assertion, not a measurement.
- A derived block now sits above the index, regenerated by `make ux-status`: scenario
  count, status and verdict distributions, the audit-date histogram, how many are stale
  and by how much, and how many carry an anchor. Every number is counted from the table.
- **Ages are measured against the block's own stamp, not the clock.** A block that aged
  on its own would turn CI red on a day nobody changed anything, and a gate that fires
  without a cause is one people learn to ignore. It goes stale when the *table* changes.
- Three ratchets. The block must agree with the table; every index row must carry both a
  date and a verdict, so a row saying only `implemented` cannot pass as verified; and the
  count of stale verifications is a ceiling that may fall but not rise — lowering it is
  what a batch of `/ux-audit` produces, raising it is a decision that costs a sentence in
  a diff.
