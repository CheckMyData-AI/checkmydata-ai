...
def upgrade() -> None:
    op.add_column(
        "db_index", sa.Column("enum_labels_json", sa.Text(), server_default="{}", nullable=False)
    )
    op.add_column(
        "db_index",
        sa.Column("check_constraints_json", sa.Text(), server_default="{}", nullable=False),
    )
    op.add_column(
        "db_index", sa.Column("sort_keys_json", sa.Text(), server_default="[]", nullable=False)
    )
    op.add_column(
        "db_index", sa.Column("column_stats_json", sa.Text(), server_default="{}", nullable=False)
    )
    op.add_column(
        "db_index",
        sa.Column("object_kind", sa.String(length=20), server_default="table", nullable=False),
    )
