...
class CodeDbSyncAnalyzer:
    ...
    def _system_prompt() -> str:
        return (
            ...
            "- A 'Code callers' section (when present) lists endpoints/services "
            "that read or write this table, with their inferred op_kind "
            "(read/write/upsert/delete). Use these to infer which filters are "
            "load-bearing in production — a column that every caller filters by "
            "is almost certainly a required_filter, and the constants those "
            "callers pass in are likely the canonical column_value_mappings.\n\n"
