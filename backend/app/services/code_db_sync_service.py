...
class CodeDbSyncService:
    ...
    async def delete_stale_tables(
        ...
        """Remove CodeDbSync rows not in *current_table_names* (T17).

        Bulk DELETE to avoid the previous N+1 ``session.delete`` pattern.
        """
        stmt = delete(CodeDbSync).where(CodeDbSync.connection_id == connection_id)
