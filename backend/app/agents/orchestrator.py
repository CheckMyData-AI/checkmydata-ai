...
from app.agents.request_clock import (
    _REQUEST_START_KEY as _REQUEST_START_KEY,  # re-exported: tests and callers import it here
)
from app.agents.request_clock import (
...
class OrchestratorAgent(BaseAgent):
    ...
    async def run(  # type: ignore[override]
        ...
        **_kwargs: Any,
        ...
        # O-07: was a typed 300 s, below every transport ceiling, so any request that
        # outlived five minutes lost its SQL results, routing and correction count the
        # moment another request arrived. The horizon is now the longest a live request
        # can run.
        self._cleanup_stale_results(longest_request_seconds())
        ...
        *,
        ...
        if chart_candidates and (
            ...
            async def _pick_one_viz(
                ...
                # O-08: `CancelledError` is not caught. It was, so a request cut off by
                # its deadline while choosing a chart swallowed the cancellation and kept
                # running; `wait_for`'s own timeout arrives as `TimeoutError`, which is
                # an `Exception` and still falls back to a table.
                except Exception:
                    logger.debug(
