...
CEILINGS: dict[str, int] = {
    ...
    # Dropping `CancelledError` makes it `except Exception`, which it does count.
    # 660 → 661 on 2026-09-17 (PRJ-04). One: the WebSocket transport now finalizes a
    # crashed run's trace, which needs the exception in hand — and it RE-RAISES, so
    # nothing is swallowed. REST and SSE already had the same handler; WS was the one
    # transport whose crashes left a trace with no session to join it to.
    ...
    # decorates, is worse than a gap in a graph.
    #
    #   metrics_store._parse_field / .flush x3 / .read_all  — five. The two that
    #       return a degraded value log at WARNING (the silent-degradation ratchet
    #       beside this one demanded it, and it was right: a field nothing writes
    #       being unreadable means a series is vanishing from the page). The three
    #       around Redis calls stay at debug deliberately — a Redis outage logging at
    #       warning once a minute trains an operator to ignore warnings, and the
    #       delta is retried on the next flush rather than lost.
    #   orphan_runs._catalog  — the catalog is a diagnostic, and one that can abort
    #       the recovery it describes is the shape N3 already argued against here.
    #   worker._metrics_flush_loop x2  — the loop measures the worker; it must not be
    #       able to stop it. The second is the final flush on cancellation, where a
    #       failure must not turn a clean shutdown into an error.
    #
    # None of them is a narrower clause in disguise: every one wraps a call into
    # redis-py or into an arbitrary collector object, where the exception type is not
    # ours to enumerate.
    ...
    # worse than the failure it recovers from.
    #   * `orphan_runs.requeue_orphaned_runs` x3 — reading the running runs, closing one
    #     row, and enqueueing its replacement. Each failure is logged at WARNING or ERROR
    #     naming the consequence, and the sweep continues to the next row; the enqueue one
    #     says outright that nothing is rebuilding that project.
    #   * `worker.startup` x1 — around session creation and the import, which sit outside
    #     the function's own "never raises" promise and so are not covered by it.
    ...
    # through three layers of generics. Spelling that out documents the library rather
    # than the code. It exists because collapsing the budget gate from six queries to
    # one needs the same conditional-sum shape six times, and six copies of it is the
