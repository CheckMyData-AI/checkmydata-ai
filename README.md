...
- **Programmatic table resolution** -- before delegating to the SQL agent, the orchestrator runs a lightweight heuristic (`table_resolver.py`) that matches user question terms against known tables via exact, plural/singular, substring, and keyword-to-description matching. Unresolvable terms trigger explicit `ask_user` clarification guidance in the prompt.
