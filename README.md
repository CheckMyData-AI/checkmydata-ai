...
**Supported databases**: PostgreSQL, MySQL, ClickHouse, MongoDB (plus file-backed SQLite, which powers the built-in demo)
...
Intelligence remediation (W0–W6), self-completing embedding reconcile, and
sync/orchestrator hardening, followed by a full UX-audit remediation
(email verification, password reset, invite decline, dashboard/batch fixes)
and the R3 cross-tenant isolation release. See
[`CHANGELOG.md`](CHANGELOG.md) for the full list.

- **Data-quality honesty** — truncation/partial-data caveats everywhere;
...
- **Hybrid retrieval + ContextPack** — BM25 ⊕ dense RRF with per-chunk
  provenance and a cross-encoder reranker (benchmark-gated, default-on).
...
- **Account lifecycle complete** — email verification, forgot/reset
  password, and invite decline shipped after the 2026-07-19 UX audit.
...
- **Billing & plans** -- Stripe-backed subscriptions (Checkout, Customer Portal, idempotent webhooks) with Free/Pro/Team plans, plan-derived entitlements (LLM token budgets, connection/project quotas → HTTP 402 with upgrade hint), a `/pricing` page, and an in-app billing panel showing plan + usage.
- **Production hardening** -- Redis-backed rate limiting / agent concurrency / WS tickets (in-memory fallback for dev), Sentry error tracking on backend + frontend with PII/secret scrubbing, SSH host-key verification (`tofu` default, fail-closed) and pre-command allowlisting, and MCP resources gated by the same principal/ownership checks as MCP tools.
- **Progressive Web App** -- installable, responsive design with mobile sidebar drawer
- **Cinematic motion system** -- GSAP 3 + ScrollTrigger + Lenis scrollytelling on the marketing pages (pinned 4-beat hero narrative, scroll word-lighting, count-up stats) and Framer Motion state choreography in the product UI (spring-animated pipeline stages with active glow / failure shake, message-list transitions, chart draw-on, staggered result-table rows). Shared motion tokens live in `frontend/src/lib/motion/tokens.ts`; everything degrades gracefully under `prefers-reduced-motion` and on touch devices. See [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md) §3.2.
...
| `/pricing` | Plans & pricing with Stripe Checkout |
...
- **~6,125 total tests** (5,580 backend + 546 frontend)
- **~78% backend coverage** (combined; CI-enforced minimum is 72%; target 80%, tracked in [BACKLOG.md](BACKLOG.md) Sprint 9)
- Zero flaky tests; 5 justified skips + 1 documented xfail
